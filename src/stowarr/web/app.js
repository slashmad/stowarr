...
const reconcilePrimaryNeedsRepair=plan=>(plan?.pairs||[]).some(pair=>!RECONCILE_UNCHANGED_PAIR_STATUSES.has(pair.status));
...
function renderReconcileBlocker(plan){
  ...
  if(plan.error_code==='SONARR_PARTIAL_SERIES_POOL_CHANGE'){
    ...
    const moveButton=details.recommended_move_pool?`<button type="button" class="primary" id="open-recommended-sonarr-move" data-hash="${esc(plan.torrent_hash)}" data-pool="${esc(details.recommended_move_pool)}">Open Move to ${esc(details.recommended_move_pool)}</button>`:'';
    ...
    return `<article class="panel release-conflict"><div class="panel-head"><div><h2>A partial season cannot move the whole Sonarr series</h2><p>Stowarr stopped before changing qBittorrent, Sonarr, or any files.</p></div>${badge('blocked')}</div><div class="release-evidence"><div><small>This qBittorrent release proves</small><strong>${details.selected_file_count??0} managed episode file${details.selected_file_count===1?'':'s'}</strong><span>${details.selected_episode_count??0} mapped episode${details.selected_episode_count===1?'':'s'}</span></div><div><small>Current Sonarr series location</small><strong>${esc(current)}</strong><code>${esc(plan.current_item_path||'Unknown')}</code></div><div><small>qBittorrent authoritative pool</small><strong>${esc(target)}</strong><span>Changing this route would change the root for the entire series.</span></div></div>${inventory}<div class="recovery-guidance"><div><strong>Recommended safe action</strong><p>Move this qBittorrent release to ${esc(current)}. The series stays together, and Reconcile can then repair only the episodes owned by this release. Stowarr creates missing destination folders when the resulting plan is safe.</p><small>A whole-series migration needs a separate plan that proves every managed episode and every associated release.</small></div>${moveButton}</div></article>`;
  }
...
function renderPlan(plan){
  ...
  $('#plan-result').innerHTML=`${error}<article class="panel"><div class="panel-head"><div><h2>Plan details</h2><p>No changes have been made</p></div></div><div class="detail-grid">${details.map(([k,v])=>`<div class="detail"><small>${k}</small><strong>${v}</strong></div>`).join('')}</div></article>${repairPanel}`
}
...
async function inspectMove(hash,targetPool){
  ...
  $('#move-target').value=targetPool;
  $('#move-loading').classList.remove('hidden');
  $('#move-result').innerHTML='';
  try{
    state.movePlan=await api(`/api/move/plan/${encodeURIComponent(hash)}?targetPool=${encodeURIComponent(targetPool)}`);
    renderMovePlan(state.movePlan);
    initializeMoveActions(state.movePlan);
    enhanceMoveRecovery(state.movePlan);
  }catch(e){
