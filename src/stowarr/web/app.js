...
const RECONCILE_ACTIONABLE_AUXILIARY_STATUSES=new Set(['torrent-sidecar','missing-target','target-exists-same-size','stale-linked','stale-target-conflict']);
const reconcilePrimaryNeedsRepair=plan=>{
  ...
  return Boolean(plan?.sonarr_import_files?.length)||Boolean(current&&target&&current!==target)||(plan?.pairs||[]).some(pair=>!RECONCILE_UNCHANGED_PAIR_STATUSES.has(pair.status));
};
...
const RECONCILE_PROGRESS=[['PLANNED','Plan accepted','The reviewed repair plan has been registered.'],['RECONCILE_VERIFYING','Library files verifying','Source content is hash-verified before links or copies are created.'],['RECONCILE_SONARR_IMPORTING','Sonarr episode import','Sonarr imports the reviewed assignments in Copy mode; Stowarr then verifies hardlink identity.'],['LINKED','Library files created','Verified media is present on the authoritative pool.'],['AUXILIARY_COPIED','Sidecars copied','Selected subtitles, artwork, and metadata have been verified.'],['ARR_UPDATED','*Arr route updated','The movie or series points at the authoritative pool.'],['ARR_RESCANNING','*Arr library rescanning','Stowarr waits for Radarr or Sonarr to finish.'],['ARR_RESCANNED','*Arr rescan verified','Managed files are confirmed at their expected paths.'],['SOURCE_UNLINKED','Stale source removed','Verified obsolete files are removed last.'],['COMPLETE','Reconcile complete','The library and qBittorrent agree.']];
...
const INDETERMINATE_PROGRESS_STATES=new Set(['MOVE_RELOCATING','MOVE_ARR_RESCANNING','ARR_RESCANNING','RECONCILE_SONARR_IMPORTING','MOVE_RESUMING','MOVE_SEEDING']);
...
function renderOperationDialog(operation,waiting=false,updateTrackedOperation=true){
  ...
  const progressSteps=isReconcile?RECONCILE_PROGRESS.filter(([name])=>name!=='RECONCILE_SONARR_IMPORTING'||Boolean(operation?.detail?.sonarr_import_files?.length)):isCategory?CATEGORY_PROGRESS:MOVE_PROGRESS.filter(([name])=>hasArchive||!ARCHIVE_PROGRESS_STATES.has(name));
  ...
  const update=async()=>{
    ...
    try{
      ...
      if(operation){
        ...
        if(operation.state==='COMPLETE'&&operation.kind==='reconcile'&&operation.app&&!state.auditRefreshedOperations.has(operation.id)){
          state.auditRefreshedOperations.add(operation.id);
          await runSync({app:operation.app});
        }
...
function cleanupOwnershipLabel(value){return {'both':'*Arr + qBit','arr-only':'*Arr only','arr-content-only':'*Arr file only','managed-content':'*Arr + qBit files','qbittorrent-only':'qBit only','unowned':'Unowned','ambiguous':'Ambiguous','arr-managed':'*Arr managed','qbittorrent':'qBit'}[value]||value}
function cleanupBadge(value){return `<span class="badge ${esc(value)}">${esc(cleanupOwnershipLabel(value))}</span>`}
function renderCleanupInventory(){
  const inventory=state.cleanupInventory;if(!inventory)return;const summary=inventory.summary||{};const statuses=summary.statuses||{};
  $('#cleanup-summary').innerHTML=`<div><strong>${summary.folders||0}</strong><small>Title folders</small></div><div><strong>${summary.review_recommended||0}</strong><small>Review recommended</small></div><div><strong>${statuses.unowned||0}</strong><small>Unowned</small></div><div><strong>${statuses['qbittorrent-only']||0}</strong><small>qBit only</small></div>`;
  $('#cleanup-warning').innerHTML=inventory.complete?'':`<div class="alert inline-alert"><div class="alert-head">Inventory is incomplete</div><p>${esc((inventory.warnings||[]).length)} API or filesystem reads failed. Do not make cleanup decisions from this scan alone.</p></div>`;
  const roots=(inventory.roots||[]).map(root=>{const folders=[...(root.folders||[])].sort((a,b)=>Number(b.review_recommended)-Number(a.review_recommended)||a.name.localeCompare(b.name));const folderRows=folders.map(folder=>{const counts=folder.counts||{};const arr=(folder.arr_items||[]).map(item=>`${item.app==='radarr'?'Radarr':'Sonarr'}: ${item.title||'#'+item.id}`).join(' · ')||'No exact *Arr item path';const torrents=(folder.torrents||[]).map(item=>`${item.name} · ${item.hash.slice(0,12)}`).join(' · ')||'No selected qBittorrent file or hardlink';const contents=(folder.contents||[]).map(item=>`<tr><td>${cleanupBadge(item.ownership)}</td><td class="path">${esc(item.path)}</td><td>${esc(item.kind)}</td><td>${fmtBytes(item.size)}</td><td>${item.arr?.length?esc(item.arr.map(owner=>`${owner.app} #${owner.id}`).join(', ')):'—'}</td><td>${item.torrents?.length?esc(item.torrents.map(owner=>owner.hash.slice(0,12)).join(', ')):'—'}</td></tr>`).join('');return `<details class="cleanup-folder ${folder.review_recommended?'review':''}"><summary><span>${cleanupBadge(folder.status)}<strong>${esc(folder.name)}</strong></span><span>${counts.files||0} files · ${fmtBytes(counts.bytes)}${folder.review_recommended?' · review':''}</span></summary><div class="cleanup-folder-meta"><code>${esc(folder.path)}</code><small>${esc(arr)}</small><small>${esc(torrents)}</small><small>${counts.unowned||0} unowned entries · ${counts.unmanaged_video||0} unmanaged videos · ${counts.directories||0} subdirectories</small></div><div class="table-wrap"><table><thead><tr><th>Ownership</th><th>Relative path</th><th>Kind</th><th>Size</th><th>*Arr</th><th>qBit hash</th></tr></thead><tbody>${contents||'<tr><td colspan="6" class="empty">The folder is empty</td></tr>'}</tbody></table></div></details>`}).join('');const stateLabel=root.state==='available'?`${folders.length} folders`:root.state;return `<article class="panel cleanup-root"><div class="panel-head"><div><h2>${esc(root.pool)} · ${esc(root.app==='radarr'?'Radarr':'Sonarr')}</h2><p class="path">${esc(root.path)}</p></div>${badge(stateLabel)}</div>${folderRows||'<div class="empty">No title folders found in this root</div>'}</article>`}).join('');
  $('#cleanup-results').innerHTML=roots||'<article class="panel"><div class="empty">No configured library roots were found</div></article>';
}
async function runCleanupInventory(){const button=$('#run-cleanup');button.disabled=true;button.textContent='Scanning…';$('#cleanup-loading').classList.remove('hidden');$('#cleanup-warning').innerHTML='';try{state.cleanupInventory=await api('/api/cleanup/inventory');renderCleanupInventory();toast(`Cleanup inventory found ${state.cleanupInventory.summary.folders} folders`)}catch(e){$('#cleanup-warning').innerHTML=`<div class="alert"><div class="alert-head">Inventory failed</div><p>${esc(e.message)}</p></div>`}finally{$('#cleanup-loading').classList.add('hidden');button.disabled=false;button.textContent='Scan library folders'}}
...
function reconcileActionCopy(plan){
  ...
  if(plan.sonarr_import_files?.length)return{
    title:'Ready for Sonarr-assisted episode import',
    message:'Stowarr will force-recheck qBittorrent, ask Sonarr to import the reviewed file-to-episode assignments in Copy mode, and require every library result to be a hardlink to the torrent source.',
    button:'Import episodes with Sonarr',
    confirmTitle:`Import ${plan.sonarr_import_files.length} episode file${plan.sonarr_import_files.length===1?'':'s'} for ${plan.item_title}?`,
    confirmMessage:'Every displayed assignment is bound to this confirmation. qBittorrent files are retained, and a separate copy fails verification.',
    detailLabel:'Sonarr episode imports',
  };
...
function renderSonarrImports(files){
  if(!files?.length)return '';
  return `<details class="move-manifest" open><summary><strong>${files.length} Sonarr import candidate${files.length===1?'':'s'}</strong><span>Every selected qBittorrent video has one reviewed episode assignment</span></summary><div class="table-wrap"><table><thead><tr><th>qBittorrent source</th><th>Sonarr episode IDs</th><th>Size</th><th>Mode</th></tr></thead><tbody>${files.map(file=>`<tr><td class="path">${esc(file.path)}</td><td>${file.episodeIds.map(id=>`#${esc(id)}`).join(', ')}</td><td>${fmtBytes(file.size)}</td><td>${badge('copy / hardlink required')}</td></tr>`).join('')}</tbody></table></div></details>`;
}
function renderPlan(plan){
  ...
  const pairs=plan.app==='sonarr'?`${renderSonarrImports(plan.sonarr_import_files)}${plan.pairs?.length?renderSonarrEpisodePairs(plan.pairs):plan.sonarr_import_files?.length?'':'<div class="empty">No managed episode files are part of this release</div>'}`:(plan.pairs?.length?plan.pairs.map(pair=>renderReconcilePair(pair,plan.app)).join(''):'<div class="empty">No file pairs available</div>');
  ...
  const staleAux=(plan.auxiliary_files||[]).filter(x=>x.operation==='delete');
  ...
  const auxiliary=plan.auxiliary_files?.length?`<div class="auxiliary"><div class="auxiliary-head"><span class="section-kicker">${plan.app==='sonarr'?'Optional series files':'Optional secondary files'}</span><strong>Subtitles, NFO, artwork, and other non-video files</strong><small>Video and episode files are never treated as optional secondary files.</small></div><label class="check-option select-all"><input id="select-all-auxiliary" type="checkbox" ${eligibleAux.length?'checked':'disabled'}><span><strong>${eligibleAux.length?`Select all ${eligibleAux.length} secondary and cleanup actions`:'No secondary files require repair'}</strong><small>qBittorrent-owned files are hardlinked. Old-library files are copied when missing; explicitly listed stale metadata is removed only after *Arr verification.</small></span></label><details ${plan.auxiliary_files.length<=40?'open':''}><summary>${plan.auxiliary_files.length} secondary files · ${esc(auxKinds)} · ${missingAux.length} missing${linkedAux.length?` · ${linkedAux.length} already linked`:''}${staleAux.length?` · ${staleAux.length} stale cleanup`:''}${conflicts.length?` · ${conflicts.length} unresolved conflict`:''}</summary><div class="aux-list">${plan.auxiliary_files.map(x=>{const selectable=reconcileAuxiliaryNeedsRepair(x);const origin=x.origin==='qbittorrent'?(x.status==='linked'?'qBittorrent · already linked':'qBittorrent · hardlink'):(x.operation==='delete'?'Old library · remove after verification':'Old library · copy');return `<label class="aux-row ${selectable?'':'disabled'}"><input class="aux-file" type="checkbox" data-source="${esc(x.source)}" ${selectable?'checked':'disabled'}><span class="origin">${esc(origin)}</span><span>${badge(x.status)}</span><code class="path">${esc(x.source)}</code><span>→</span><code class="path">${esc(x.target)}</code></label>`}).join('')}</div></details>${staleAux.length?`<div class="aux-warning">${staleAux.length} old metadata file(s) are explicit cleanup actions. Stowarr rechecks that they are non-video, outside qBittorrent ownership, inside the old library root, and backed by an existing target before unlinking them.</div>`:''}${conflicts.length?`<div class="aux-warning">${conflicts.length} secondary file(s) still have an unresolved destination conflict. They are skipped and will not be overwritten or deleted automatically.</div>`:''}</div>`:'';
  ...
  const canVerify=hasWork&&!plan.sonarr_import_files?.length;
  const enabled=apply&&hasWork;
  const action=ready?`<div id="verification-result"></div><div class="reconcile-action"><div><strong>${hasWork?(apply?esc(copy.title):'Changes are locked in dry-run mode'):'No repairs are needed'}</strong><p>${hasWork?(apply?esc(copy.message):'Enable Write mode and provide writable media mounts only after reviewing the plan.'):'Every managed media file and discovered secondary file already has the expected hardlink identity.'}</p></div><div class="action-buttons"><button id="verify-plan" class="secondary" ${canVerify?'':'disabled'}>${plan.sonarr_import_files?.length?'Sonarr preview verified':'Verify content hashes'}</button><button id="queue-reconcile" class="secondary" ${enabled?'':'disabled'}>Add to queue</button><button id="apply-plan" class="danger" ${enabled?'':'disabled'}>${hasWork?esc(copy.button):'Nothing to repair'}</button></div></div>`:'';
...
async function applyPlan(){
  ...
  const staleCleanup=(plan.auxiliary_files||[]).filter(item=>item.operation==='delete'&&auxiliaryFiles.includes(item.source)).length;
  const primaryDetail=plan.sonarr_import_files?.length?`${plan.sonarr_import_files.length} exact file-to-episode assignments`:(copy.mediaUntouched?'None — existing file kept':oldFiles);
  if(!await confirmAction({title:copy.confirmTitle,message:copy.confirmMessage,details:[[copy.detailLabel,primaryDetail],['Selected secondary actions',String(auxiliaryFiles.length)],['Old metadata files removed after verification',String(staleCleanup)]],confirmLabel:copy.button,danger:true}))return;
  ...
  try{
    ...
    await load();
    if(!state.auditRefreshedOperations.has(result.operation_id)){
      state.auditRefreshedOperations.add(result.operation_id);
      await runSync({app:plan.app});
    }
  }catch(e){
    registrationWatcher?.stop();
    await refreshOperations();
...
async function enqueueReconcile(){const plan=state.plan;if(!plan||plan.status!=='ready'||!state.config?.apply||!reconcileHasSelectedWork(plan))return;const auxiliaryFiles=$$('.aux-file:checked').map(input=>input.dataset.source);const staleCleanup=(plan.auxiliary_files||[]).filter(item=>item.operation==='delete'&&auxiliaryFiles.includes(item.source)).length;if(!await confirmAction({title:`Add ${plan.item_title} to the Reconcile queue?`,message:'The plan is revalidated immediately before execution.',details:[['Selected secondary actions',String(auxiliaryFiles.length)],['Old metadata files removed after verification',String(staleCleanup)],['Sonarr episode imports',String(plan.sonarr_import_files?.length||0)]],confirmLabel:'Add to queue'}))return;const button=$('#queue-reconcile');button.disabled=true;button.textContent='Authorizing…';try{const confirmation=await api('/api/confirmations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({kind:'reconcile',torrentHash:plan.torrent_hash,payload:{auxiliaryFiles}})});const queued=await api('/api/reconcile-queue',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({torrentHash:plan.torrent_hash,auxiliaryFiles,confirmationToken:confirmation.token})});toast(`Reconcile queued as ${queued.public_id}`);await refreshQueue();navigate('queue')}catch(error){toast(`Reconcile was not queued: ${error.message}`);button.disabled=false;button.textContent='Add to queue'}}
