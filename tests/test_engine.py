...
class EngineTest(unittest.TestCase):
    def test_cleanup_inventory_uses_arr_paths_and_qbit_hardlink_identity(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            both_folder = movies / "Masked Both"
            arr_only_folder = movies / "Masked Arr"
            qbit_only_folder = movies / "Masked Old"
            ignored_folder = movies / "Masked Ignored"
            empty_folder = movies / "Masked Empty"
            for folder in (
                both_folder, arr_only_folder, qbit_only_folder,
                ignored_folder, empty_folder,
            ):
                folder.mkdir()

            both_source = downloads / "both.mkv"
            old_source = downloads / "old.mkv"
            ignored_source = downloads / "ignored.mkv"
            both_source.write_bytes(b"both")
            old_source.write_bytes(b"old")
            ignored_source.write_bytes(b"ignored")
            both_file = both_folder / "movie.mkv"
            old_file = qbit_only_folder / "old-name.mkv"
            ignored_file = ignored_folder / "ignored-name.mkv"
            os.link(both_source, both_file)
            os.link(old_source, old_file)
            os.link(ignored_source, ignored_file)
            arr_only_file = arr_only_folder / "movie.mkv"
            arr_only_file.write_bytes(b"arr")

            radarr_items = [
                {
                    "id": 1, "title": "Masked Both", "path": str(both_folder),
                    "movieFile": {"id": 10, "path": str(both_file)},
                },
                {
                    "id": 2, "title": "Masked Arr", "path": str(arr_only_folder),
                    "movieFile": {"id": 20, "path": str(arr_only_file)},
                },
            ]
            radarr = SimpleNamespace(
                all_items=lambda: radarr_items,
                managed_files=lambda item: [item["movieFile"]],
                root_folders=lambda: [{"path": str(movies)}],
            )
            sonarr = SimpleNamespace(
                all_items=lambda: [], managed_files=lambda item: [],
                root_folders=lambda: [{"path": str(series)}],
            )
            qbit_files = [
                {"name": both_source.name, "priority": 1},
                {"name": old_source.name, "priority": 1},
                {"name": ignored_source.name, "priority": 0},
            ]
            qbit = SimpleNamespace(
                torrents=lambda: [{
                    "hash": "ABC123", "name": "Masked Release",
                    "save_path": str(downloads),
                }],
                files=lambda torrent_hash: qbit_files,
            )
            pool = Pool(
                "p3", root, (downloads,), movies, series,
                "radarr-p3", "sonarr-p3", "p3", "p3",
            )
            manager = Stowarr.__new__(Stowarr)
            manager.config = SimpleNamespace(pools=(pool,))
            manager.arr = {"radarr": radarr, "sonarr": sonarr}
            manager.qbit = qbit

            inventory = manager.cleanup_inventory()

            rows = {
                row["name"]: row
                for root_row in inventory["roots"]
                for row in root_row["folders"]
            }
            self.assertTrue(inventory["complete"])
            self.assertEqual(rows["Masked Both"]["status"], "both")
            self.assertEqual(
                rows["Masked Both"]["contents"][0]["ownership"], "both"
            )
            self.assertEqual(rows["Masked Arr"]["status"], "arr-only")
            self.assertFalse(rows["Masked Arr"]["review_recommended"])
            self.assertEqual(rows["Masked Old"]["status"], "qbittorrent-only")
            self.assertTrue(rows["Masked Old"]["review_recommended"])
            self.assertEqual(rows["Masked Ignored"]["status"], "unowned")
            self.assertEqual(rows["Masked Empty"]["status"], "unowned")

    def test_sonarr_import_preview_requires_one_candidate_per_torrent_video(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            video = root / "Fictional.Series.S01E01.mkv"
            video.write_bytes(b"episode")
            torrent = {"save_path": str(root), "content_path": str(root)}
            records = [{
                "name": video.name,
            ...
            manager.arr = {"sonarr": SimpleNamespace(
                manual_import_preview=Mock(return_value=preview)
            )}

            imports, error, evidence = manager._sonarr_import_candidates(
                "HASH", torrent, records, {"id": 7}
            )

            self.assertIsNone(error)
            self.assertEqual(imports[0]["episodeIds"], [70])
            self.assertEqual(imports[0]["downloadId"], "HASH")
            self.assertEqual(evidence["import_count"], 1)

            preview[0]["rejections"] = [{"reason": "Unknown episode"}]
            imports, error, evidence = manager._sonarr_import_candidates(
                "HASH", torrent, records, {"id": 7}
            )
            self.assertEqual(imports, [])
            self.assertIn("safe episode assignment", error)
            self.assertEqual(len(evidence["problems"]), 1)

    def test_sonarr_audit_promotes_a_complete_import_preview_to_safe_plan(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            video = root / "Fictional.Series.S01E01.mkv"
            video.write_bytes(b"episode")
            torrent = {
                "hash": "HASH", "save_path": str(root),
                "content_path": str(root),
            }
            item = {"id": 7, "title": "Fictional Series"}
            preview = [{
                "path": str(video), "size": video.stat().st_size,
                "series": {"id": 7}, "episodes": [{"id": 70}],
                "quality": {"quality": {"id": 1}},
                "languages": [], "rejections": [],
            }]
            manager = Stowarr.__new__(Stowarr)
            manager.arr = {"sonarr": SimpleNamespace(
                download_mapping=lambda torrent_hash: {
                    "item": item, "mappingComplete": False, "files": [],
                },
                manual_import_preview=Mock(return_value=preview),
            )}
            manager.qbit = SimpleNamespace(files=lambda torrent_hash: [{
                "name": video.name, "size": video.stat().st_size,
                "priority": 1,
            }])

            status, reason, issues, count = manager._sonarr_sync_media_status(
                torrent, item
            )

            self.assertEqual(status, "sonarr-import-ready")
            self.assertIn("safely import", reason)
            self.assertEqual(issues[0]["code"], "SONARR_IMPORT_READY")
            self.assertEqual(count, 1)
            self.assertTrue(safe_sync_candidate({"status": status}))

    def test_sonarr_import_verification_requires_hardlink_identity(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            mapping = {
                ...
                "item": {"id": 7},
                "files": [{
                    "id": 700, "path": str(linked),
                    "size": linked.stat().st_size, "episodeIds": [70],
                }],
            }

            Stowarr._verify_sonarr_imports(imports, mapping, 7)

            mapping["files"][0]["path"] = str(copied)
            with self.assertRaisesRegex(RuntimeError, "separate copy"):
                Stowarr._verify_sonarr_imports(imports, mapping, 7)

    def test_reconcile_runs_reviewed_sonarr_import_and_verifies_hardlink(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            download_root = root / "download"
            series_root = root / "series" / "Fictional"
            source = download_root / "Fictional.S01E01.mkv"
            target = series_root / "Season 01" / "Fictional.S01E01.mkv"
            download_root.mkdir()
            series_root.mkdir(parents=True)
            source.write_bytes(b"episode")
            item = {
                "id": 7, "title": "Fictional", "path": str(series_root),
                "tags": [],
            }
            incomplete = {
                "app": "sonarr", "item": item, "files": [],
                "mappingComplete": False,
            }
            complete = {
                "app": "sonarr", "item": item, "mappingComplete": True,
                "files": [{
                    "id": 700, "path": str(target),
                    "relativePath": str(target.relative_to(series_root)),
                    "size": source.stat().st_size, "episodeIds": [70],
                }],
            }
            current_mapping = [incomplete]

            def manual_import(files):
                self.assertNotIn("size", files[0])
                target.parent.mkdir(parents=True)
                os.link(source, target)
                current_mapping[0] = complete

            client = SimpleNamespace(
                manual_import=Mock(side_effect=manual_import),
                download_mapping=lambda torrent_hash: current_mapping[0],
                sync_pool=Mock(),
                rescan=Mock(),
            )
            pool = Pool(
            ...
            manager.qbit = SimpleNamespace(
                ...
                torrent=lambda torrent_hash: {
                    "hash": torrent_hash, "save_path": str(download_root),
                    "category": "sonarr-p3", "progress": 1,
                    "state": "pausedUP",
                },
            ...
            manager._wait_for_recheck = Mock()
            manager._wait_for_visible_torrent_files = Mock()
            plan = Plan(
                "IMPORT", "Fictional.S01", "sonarr", "p3", 7,
                "Fictional", str(series_root), str(series_root), [], "ready",
                sonarr_import_files=[{
                    "path": str(source), "folderName": "Fictional.S01",
                    "seriesId": 7, "episodeIds": [70],
                    "quality": {"quality": {"id": 1}}, "languages": [],
                    "releaseGroup": "", "indexerFlags": 0,
                    "releaseType": "unknown", "downloadId": "IMPORT",
                    "size": source.stat().st_size,
                }],
            )

            result = manager.reconcile(
                "IMPORT", operation_id=12, mapping_hint=incomplete,
                prepared_plan=plan,
            )

            self.assertEqual(result["state"], "COMPLETE")
            manager.qbit.recheck.assert_called_once_with("IMPORT")
            client.manual_import.assert_called_once()
            client.rescan.assert_called_once_with(7)
            self.assertEqual(
    ...
    def test_service_status_reports_live_versions_without_credentials(self):
        ...
        self.assertEqual(result["version"], "1.0.0-beta.5")
    ...
    def test_reconcile_removes_selected_stale_metadata_after_arr_verification(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            download_root = root / "download"
            old_root = root / "old" / "Fictional (2024)"
            target_root = root / "new" / "Fictional (2024)"
            download_root.mkdir()
            old_root.mkdir(parents=True)
            target_root.mkdir(parents=True)
            stale = old_root / "poster.jpg"
            target = target_root / "poster.jpg"
            stale.write_bytes(b"old artwork")
            target.write_bytes(b"new artwork is different")
            item = {
                "id": 77, "title": "Fictional", "path": str(old_root),
                "tags": [],
            }
            mapping = {"app": "radarr", "item": item, "files": []}
            plan = Plan(
                "CLEAN", "Fictional.2024", "radarr", "p3", 77,
                "Fictional", str(old_root), str(target_root), [], "ready",
                auxiliary_files=[AuxiliaryFile(
                    str(stale), str(target), stale.stat().st_size,
                    "stale-target-conflict", "library", "delete", "artwork",
                )],
            )
            pool = Pool(
                "p3", root, (download_root,), root / "movies", root / "series",
            ...
            manager.qbit = SimpleNamespace(
                torrent=lambda torrent_hash: {
                    "save_path": str(download_root), "hash": torrent_hash,
                },
                files=lambda torrent_hash: [],
            )

            result = manager.reconcile(
                "CLEAN", {str(stale)}, operation_id=10,
                mapping_hint=mapping, prepared_plan=plan,
            )
            ...
            self.assertEqual(target.read_bytes(), b"new artwork is different")
            client.rescan.assert_called_once_with(77)

    def test_stale_metadata_cleanup_rejects_qbittorrent_owned_source(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            old_root = root / "old"
            ...
            mapping = {
                "item": {"id": 7, "title": "Fictional", "path": str(old_root), "tags": []},
                "files": [],
            }
            plan = Plan(
                "OWNED", "Fictional", "radarr", "p3", 7, "Fictional",
                str(old_root), str(target_root), [], "ready",
                auxiliary_files=[AuxiliaryFile(
                    str(stale), str(target), stale.stat().st_size,
                    "stale-target-conflict", "library", "delete", "artwork",
                )],
            ...
            manager.arr = {"radarr": SimpleNamespace(
                download_mapping=lambda torrent_hash: mapping,
                sync_pool=Mock(), rescan=Mock(),
            )}
            manager.config = SimpleNamespace(
                apply=True,
                pools=(Pool(
                    "p3", root, (old_root,), root / "movies", root / "series",
                    "radarr-p3", "sonarr-p3", "radarr-p3", "sonarr-p3",
                ),),
            )
            manager.store = SimpleNamespace(update=Mock())
            manager.qbit = SimpleNamespace(
                torrent=lambda torrent_hash: {"save_path": str(root)},
                files=lambda torrent_hash: [{
                    "name": str(stale.relative_to(root)),
            ...
            with self.assertRaisesRegex(RuntimeError, "changed before cleanup"):
                manager.reconcile(
                    "OWNED", {str(stale)}, operation_id=11,
                    mapping_hint=mapping, prepared_plan=plan,
                )
    ...
    def test_safe_sync_plan_keeps_stale_metadata_cleanup_for_manual_review(self):
        row = {
            "hash": "CLEANUP", "torrent_name": "Fictional Example",
            "item_title": "Fictional Example", "status": "root-mismatch",
            "reason": "wrong root", "issues": [],
        }
        plan = Plan(
            "CLEANUP", "Fictional Example", "radarr", "p3", 1,
            "Fictional Example", "/old/Fictional", "/new/Fictional",
            [], "ready",
            auxiliary_files=[AuxiliaryFile(
                "/old/Fictional/poster.jpg", "/new/Fictional/poster.jpg",
                10, "stale-target-conflict", "library", "delete", "artwork",
            )],
        )
        manager = Stowarr.__new__(Stowarr)
        manager.sync_audit = Mock(return_value={
            "app": "radarr", "scanned": 1, "issues": 1, "rows": [row],
        })
        manager.plan = Mock(return_value=plan)
        manager.store = SimpleNamespace(reconcile_queue=lambda: [])

        result = manager.safe_sync_plan("radarr")

        self.assertEqual(result["reconcile_candidates"], [])
        self.assertEqual(result["manual"][0]["error_code"], "STALE_METADATA_REVIEW_REQUIRED")
